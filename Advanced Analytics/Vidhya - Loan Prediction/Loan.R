...
FeatEngAftImp <- function(df_all_x){
  ##Feature Engineering after Imputation
  #Full Year
  df_all_x$FullYearTerm <- 0
  df_all_x$FullYearTerm[df_all_x$Loan_Amount_Term == 360] <- 1
  
  df_all_x$HalfYearTerm <- 0
  df_all_x$HalfYearTerm[df_all_x$Loan_Amount_Term == 180] <- 1
  
  df_all_x$ThirdofYearTerm <- 0
  df_all_x$ThirdofYearTerm[df_all_x$Loan_Amount_Term == 120] <- 1
  
  df_all_x$ShortTerm <- 0
  df_all_x$ShortTerm[df_all_x$Loan_Amount_Term < 120] <- 1
  
  #Instalments
  df_all_x$DailyInstalments <- 0
  df_all_x$DailyInstalments <- df_all_x$LoanAmount/df_all_x$Loan_Amount_Term
  
  df_all_x$WeeklyInstalments <- 0
  df_all_x$WeeklyInstalments <- df_all_x$DailyInstalments * 7
  
  df_all_x$MonthlyInstalments <- 0
  df_all_x$MonthlyInstalments <- df_all_x$DailyInstalments * 30
  
  #TIncomeExcLoan
  df_all_x$TIncomeExcLoan <- df_all_x$TotalIncome / df_all_x$LoanAmount
  
  #AIncomeExcLoan
  df_all_x$AIncomeExcLoan <- df_all_x$ApplicantIncome / df_all_x$LoanAmount
  
  #MarriedGender
  df_all_x$GendMar <- df_all_x$Gender + df_all_x$Married
  
  #FamilySize
  df_all_x$FamilySize <- (df_all_x$Married+1) + df_all_x$Dependents
  
  #Occupation
  df_all_x$Entrepreneur <- 0
  df_all_x$Entrepreneur[df_all_x$Education == 1 & df_all_x$Self_Employed == 1] <- 1
  
  df_all_x$ShopOwner <- 0
  df_all_x$ShopOwner[df_all_x$Education == 0 & df_all_x$Self_Employed == 1] <- 1
  
  df_all_x$Corporate <- 0
  df_all_x$Corporate[df_all_x$Education == 1 & df_all_x$Self_Employed == 0] <- 1
  
  df_all_x$BlueCollar <- 0
  df_all_x$BlueCollar[df_all_x$Education == 0 & df_all_x$Self_Employed == 0] <- 1
  
  #Applicants Relationship
  df_all_x$SpouseNotWorking <- 0
  df_all_x$SpouseNotWorking[df_all_x$Married == 1 & df_all_x$CoapplicantIncome == 0] <- 1
  
  #Logarithm transformation and scaling
  df_all_x$ApplicantIncome <- log1p(df_all_x$ApplicantIncome)
  df_all_x$CoapplicantIncome <- log1p(df_all_x$CoapplicantIncome)
  df_all_x$LoanAmount <- log1p(df_all_x$LoanAmount)
  df_all_x$Loan_Amount_Term <- log1p(df_all_x$Loan_Amount_Term)
  df_all_x$TotalIncome <- log1p(df_all_x$TotalIncome)
  df_all_x$DailyInstalments <- log1p(df_all_x$DailyInstalments)
  df_all_x$WeeklyInstalments <- log1p(df_all_x$WeeklyInstalments)
  df_all_x$MonthlyInstalments <- log1p(df_all_x$MonthlyInstalments)
  df_all_x$TIncomeExcLoan <- log1p(df_all_x$TIncomeExcLoan)
  df_all_x$AIncomeExcLoan <- log1p(df_all_x$AIncomeExcLoan)
  df_all_x <- data.frame(scale(df_all_x))
...
###Pre-processing
#Total Income
df_all_raw$TotalIncome = df_all_raw$ApplicantIncome + df_all_raw$CoapplicantIncome
...
#NAs with Mice
imp <- mice(df_all_raw[, 1:(ncol(df_all_raw)-3)], m = 3, method = "pmm")

#Imputation
df_all_x <- cbind(complete(imp, 1), df_all_raw$df_all_rawSemiurban, df_all_raw$df_all_rawUrban, df_all_raw$df_all_rawRural)
df_all_x <- FeatEngAftImp(df_all_x)
...
l_auc <- list()
for (i in c(0, 0, 1, 2, 3, 4, 5, 6, 7, 7.5, 8, 9)){
...
for (i in 1:imp$m){
  ...
  df_all_x <- cbind(complete(imp, i), df_all_raw$df_all_rawSemiurban, df_all_raw$df_all_rawUrban, df_all_raw$df_all_rawRural)
  df_all_x <- FeatEngAftImp(df_all_x)
  ...
  df_train_sel <- cbind(df_train_all[, which(varImp(fullModel)[1] > 3)], Loan_Status = df_train_all$Loan_Status)
  df_test_sel <- df_test_x[, which(varImp(fullModel)[1] > 3)]
  ...
  gridControl <- expand.grid(nrounds = c(200),
                             max_depth = c(1),
                             eta = c(15e-2),
                             gamma = c(1),
