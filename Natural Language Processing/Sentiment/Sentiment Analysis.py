...
class sentiment(object):
    ...
    def __init__(self, p, sp, n):
        ...
        self.feats_train = self.feat_create(self.l_train, top = 1000)
        self.feats_test = self.feat_create(self.l_test, top = 1000)
    ...
    def list_txt(self, path, split = 0.95, pn = False):
        ...
        for i, filename in enumerate(os.listdir(path)): 
            reviews.insert(
                    i, 
                    [list(tokenizer.tokenize(open(path + filename).read())), pn]
                    )
...
if __name__ == "__main__":
    analysis = sentiment(p = "F:/Code/Sentiment/neg/",
                         n = "F:/Code/Sentiment/pos/",
                         sp = 0.95)
